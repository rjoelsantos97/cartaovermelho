    ...
    <div className="min-h-screen bg-gray-50 p-8">
      <div className="max-w-6xl mx-auto">
        ...
        <div className="grid grid-cols-1 xl:grid-cols-3 lg:grid-cols-2 gap-8">
          ...
          <Card>
            <CardHeader>
              <CardTitle>EXECUTAR PIPELINE</CardTitle>
            </CardHeader>
          ...
          <Card>
            <CardHeader>
              <CardTitle>SCHEDULER AUTOMÁTICO</CardTitle>
            </CardHeader>
            <CardContent className="space-y-4">
              <p className="text-sm text-gray-600">
                ...
                <div className="space-y-4">
                  {result.success ? (
                      ...
                      {result.pipeline && (
                        <div className="grid grid-cols-3 gap-4 text-center">
                          <div className="bg-blue-50 p-3 rounded">
                            ...
                            <div className="text-sm text-blue-700">Scraped</div>
                          </div>
                    ...
                    <div className="flex items-center gap-2 text-red-600">
                      ...
                      <span className="font-semibold">Erro: {result.error}</span>
                    </div>
                  )}
                </div>
              ) : (
                <p className="text-gray-500 text-center py-8">
                  Execute o pipeline para ver resultados
                </p>
              )}
            </CardContent>
          </Card>
        ...
        {/* Sample Articles */}
        {(result?.sampleArticles || status?.sampleArticles) && (
          <Card className="mt-8">
            <CardHeader>
              <CardTitle>ARTIGOS PROCESSADOS</CardTitle>
            </CardHeader>
            <CardContent>
              <div className="space-y-4">
                {(result?.sampleArticles || status?.sampleArticles || []).map((article) => (
                  <div key={article.id} className="border rounded-lg p-4">
                    <div className="flex items-start justify-between gap-4">
                      <div className="flex-1">
                        <h3 className="font-bold text-lg mb-2">{article.dramaticTitle}</h3>
                        <div className="flex items-center gap-2">
                          <Badge variant="outline">{article.category}</Badge>
                          <Badge className={getUrgencyColor(article.urgencyLevel)}>
                            {'urgencyLevel' in article ? article.urgencyLevel : 'unknown'}
                          </Badge>
